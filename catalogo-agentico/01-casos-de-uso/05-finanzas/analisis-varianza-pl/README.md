...
> Aviso permanente: ficha de adopción. La capa de gobierno se desarrolla en la **arquitectura de remediación (bloque 5)** y aplica sin excepción.
...
**Arquetipo:** *A3 — Analítico con write-back gated* (ver [`../../arquetipos.md`](../../arquetipos.md)). El agente `pl-variance` extrae real vs. presupuesto del ERP en modo lectura, identifica varianzas materiales por CeCo/cuenta y **propone comentarios de narrativa** en una hoja de propuestas; un validador A2A `figure-validator` cuadra cada cifra contra el libro antes del handoff. El comentario nunca se pega al reporting: lo firma el controller.

### Particularizaciones de este caso

| Riesgo específico | Componente | Mecanismo específico |
|---|---|---|
| Narrativa alucinada que invente causa de la varianza | agentevals | cada comentario referencia una operación real del libro; sin referencia clicable el handoff se bloquea |
| MNPI (forecast pre-cierre, márgenes por CeCo) al LLM externo | agentgateway | column-mask sobre `margen_bruto` y `personal_coste`; el agente ve varianza absoluta, no importes salariales individuales |
| Reclasificación de partida por sesgo del modelo (mueve gasto a CAPEX) | agentgateway + kagent (OBO) | scope `pl:read`; reclasificación va a `proposals/reclass-YYYYMM.xlsx` y requiere HITL del controller con OBO |
| Acceso a dataset de forecast bajo MNPI antes del cierre | Istio | `AuthorizationPolicy` L4 restringe el MCP-warehouse a ventana post-cierre; pre-cierre solo dataset agregado |
