...
**Arquetipo:** *A3 — Analítico con write-back gated* (ver [`../../arquetipos.md`](../../arquetipos.md)). El agente `svc-analyst-sql-agent` traduce pregunta de negocio a SQL comentado. Solo `SELECT`; cualquier DDL/DML se rechaza a nivel de política, no de prompt.

### Particularizaciones de este caso

| Riesgo específico (anclaje regulatorio) | Componente | Mecanismo específico |
|---|---|---|
| `SELECT *` sobre tabla cruda con DNI/email/IBAN/salud → PII al contexto LLM externo (**GDPR art. 5/32 · AEPD notificación art. 33** 72h · **LOPDGDD**) | agentgateway + agentregistry | rol Snowflake `ANALYST_READ` publicado con grant solo sobre vistas etiquetadas `curated`; `raw_*` fuera del catálogo; DLP pre-envío bloquea columnas con patrón DNI/IBAN |
| Ejecución de `DROP`/`UPDATE`/`DELETE` por injection o alucinación (**DORA art. 9** integridad · SOX ITGC) | agentgateway | política per-tool: `execute_query` rechaza sentencias sin prefijo `SELECT/WITH`; parser AST antes de ejecución; rol de DB sin `MODIFY` |
| Alucinación de JOIN cruzado → cifra errónea presentada al comité | agentevals | validador A2A ejecuta la query en sandbox contra dataset dorado; hash del resultado cruzado contra baseline; eval set con 50 preguntas de negocio con SQL correcto conocido |
| Query pre-anuncio (revenue/churn no publicado) a LLM externo → **MAR art. 7/14 · CNMV** | agentgateway | clasificación `pre-announcement` por presencia de columnas del `fact_revenue`/`fact_guidance` → modelo on-prem forzado; ventana blackout T-30/T+2 |
| Falta de linaje prompt → query → resultado → dashboard | agentgateway + OTel | trace ID único propagado a Snowflake `QUERY_TAG`; log por invocación con hash de prompt, query, `row_count`, retención 24m |
