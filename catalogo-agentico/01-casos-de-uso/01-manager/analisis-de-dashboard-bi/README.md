...
| Notificación / envío | `mcp-teams` | `npx mcp-teams` | `digest:send` sobre el canal del equipo, **solo con aprobación humana** |
...
**Arquetipo:** *A1 — Documental multi-fuente con validador A2A* aplicado a BI (ver [`../../arquetipos.md`](../../arquetipos.md)). El agente `bi-digest` consulta vistas curadas del data warehouse + definiciones de métricas y devuelve tres variaciones top con cita a la métrica exacta y magnitud del cambio. **Nunca alcanza tablas crudas con PII.**

### Particularizaciones de este caso

| Riesgo específico (anclaje regulatorio) | Componente | Mecanismo específico |
|---|---|---|
| MCP data warehouse con scope amplio → lectura de PII, datos financieros pre-anuncio, RRHH (**GDPR art. 6 · LOPDGDD · MAR**) | agentgateway + agentregistry | `mcp-snowflake`/`mcp-bigquery` publicado con rol de solo lectura sobre vistas curadas etiquetadas `bi-curated`; tablas crudas con PII no aparecen en el catálogo del agente; consultas a schemas `raw_*` denegadas |
| Alucinación numérica en decisión material (VP escala hipótesis sin verificar) → decisión sobre datos inventados | agentevals | validador A2A: cada cifra citada debe cruzar contra `metric_id` del semantic layer; miss → línea marcada `[VALOR NO VERIFICADO]`; eval set con 50 dashboards históricos con variaciones etiquetadas |
| Datos financieros pre-anuncio (revenue, churn, márgenes) a asistente público → información privilegiada (**MAR art. 7/14 · CNMV**) | agentgateway | clasificación `pre-announcement` fuerza modelo on-prem; egress LLM externo deny; ventana blackout (T-30 a T+2 trimestre) con audit trail retenido |
| Uso de cuenta personal del manager en MCP → auditoría no distingue actividad humana vs agente | Istio ambient + agentgateway | identidad SPIFFE `svc-manager-bi-agent`; OIDC/JWT propio (no OBO del manager); logs Snowflake/Power BI muestran usuario distinto para consultas del agente |
| Envío del resumen semanal al canal del equipo sin control → publicación de cifras sensibles | agentgateway (MCP Gateway) + kagent | `mcp-teams` con scope `digest:send`; **HITL obligatorio antes de publicar**; OBO del manager |
| Coste explosivo con dashboards de 30-40 métricas consultados diariamente | agentgateway | cache TTL 1h de resultados `query_hash + snapshot_id`; rate-limit por manager; presupuesto semanal con corte |
