...
> Aviso permanente: ficha de adopción. La capa de gobierno (identidad de agente, MCP allowlist, observabilidad, coste) se desarrolla en la **arquitectura de remediación (bloque 5)** y aplica a este caso sin excepción.
...
**Arquetipo:** *A2 — Triage multi-señal con acciones sensibles gated* (ver [`../../arquetipos.md`](../../arquetipos.md)). El agente `phish-triager` reúne evidencia (cabeceras, SPF/DKIM/DMARC, reputación externa vía `mcp-virustotal`, telemetría interna Sentinel/AD), propone veredicto y **crea ticket Remedy en cola SOC**. Un sub-agente **`verdict-validator`** (A2A, SPIFFE separado) verifica el veredicto propuesto antes del handoff. Todas las acciones sobre buzones o reglas requieren firma humana con role explícito.

### Particularizaciones de este caso

| Riesgo específico (anclaje regulatorio) | Componente | Mecanismo específico |
|---|---|---|
| MCP no auditado (VT/urlscan público) exfiltra IOCs internos (**ENS · NIS2 art. 21**) | agentregistry + agentgateway | catálogo con solo MCPs versión enterprise con no-retention; `mcp-virustotal` con API key privada y `submissions:none`; egress restringido a endpoints de proveedor auditado |
| Agente actúa con cuenta del analista → no-repudio roto, SoD imposible | Istio ambient (SPIFFE) + kagent | identidad `spiffe://.../ns/soc/sa/svc-phish-triager`; Remedy MCP se autentica con `svc-soc-agent`, no con OBO del analista; audit trail distingue humano vs agente |
| Prompt injection incrustado en cuerpo del correo (**OWASP LLM01**) — atacante decide veredicto | agentgateway | cuerpo del `.eml` marcado `untrusted-content`; spotlighting; instrucciones detectadas → `escalate:human` obligatorio y no se propone veredicto |
| Shadow AI: analista despliega Claude Code con MCP Graph usando su token personal | agentregistry | inventario obligatorio; MCPs a Graph/Sentinel solo se publican al SA `svc-phish-triager`; tokens personales para Graph bloqueados por `AuthorizationPolicy` L7 en waypoint |
| Coste descontrolado por consultas VT sin caché ni rate-limit | agentgateway | rate-limit por agente (100 req/h a `mcp-virustotal`); cache TTL 24h en el gateway sobre URL/hash; presupuesto mensual con corte automático |
| PII (destinatarios, asuntos sensibles) fuera del perímetro sin DPIA (**GDPR art. 6 · LOPDGDD · ENS**) | agentgateway | `pii-redact` de remitente/destinatario antes del LLM; DPIA anexa al despliegue del agente; procesamiento forzado a modelo en `data_boundary=EU` |
| Decisión automatizada de `mark-legitimate` sobre phishing real | kagent | política `mark-legitimate:decide=deny`; el agente solo propone; ejecución requiere `role=soc-analyst-l2` con doble firma en escenario "alta confianza legítimo" |
